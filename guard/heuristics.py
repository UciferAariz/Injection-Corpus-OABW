...
SUSPICIOUS_PATTERNS: tuple[WeightedPattern, ...] = (
    ...
    WeightedPattern(
        re.compile(r"\b(disregard|override) (the )?(system|developer|previous|safety) (prompt|policy|instructions)\b", re.I),
        0.45,
    ),
    WeightedPattern(
        re.compile(
            r"\b(reveal|print|show|exfiltrate|send|forward) (the |all |current )?([\w' -]+ )?"
            r"(system prompt|api key|oauth token|token|secret|password|private messages|chat history|environment secrets)\b",
            re.I,
        ),
        0.55,
    ),
    WeightedPattern(
        re.compile(
            r"\b(send|forward|leak|exfiltrate)\b.*\b(user'?s?|private|current)\b.*"
            r"\b(api key|oauth token|token|secret|password|chat history|email)\b",
            re.I,
        ),
        0.55,
    ),
    ...
    WeightedPattern(re.compile(r"\b(invoke|run)\b.*\b(shell|command)\b.*\b(secret|environment|env)\b", re.I), 0.45),
